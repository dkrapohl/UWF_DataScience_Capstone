...
#### Time Series Diagnostics and Results {sec-methods-diagnostics-ts-diagnostics-results}
...
Where TP is the true positive rate and FP is the false positive rate. In the selected random forest model the precision is 74.7% representing a model with reasonable ability to correctly identify market "up" class at a rate better than chance.
...
I expect that the rolling period and lead/lag features generated were not important to the model because they were likely captured in these other measures. This is implied by PCA not removing them and the model indicating high value to these while muticollinearity study showed some of these to be moderately or highly correlated to other features.

The final model is usable for predicting the 5-day direction of the S&P 500 stock market index. Manual backtesting was done using a long-only approach wherein a purchase is made when the prediction first says the 5-day price will be up and sell when the prediction first says the price will drop. Not accounting for cost of the trade this model resulted in returns significantly greater than a simple buy-and-hold and could potentially provide one source for the high-risk allocation of an investment portfolio.
